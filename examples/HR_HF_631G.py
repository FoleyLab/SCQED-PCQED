...
for i in range(8):
        energy_array = np.zeros((num_roots, N_R))
        cavity_dict = {
        'omega_value' : 0.4388785987545134,
        ...
        np.save("fci_cavity_arrays_HF_6311G" + str(lambda_list[i]).replace(".", "_") , energy_array)
        np.save("dipoles_HF"+str(lambda_list[i]).replace(".","_"), Dipoles)
        energy_lists = energy_array.tolist()
        r_list = r_array.tolist()
        dictionary = {
        "system": "HF" + "_" +  str(lambda_list[i]).replace(".", "_"), # <== entered manually
        "r_data" : r_list, 
...
np.save("fci_r_array_HF_6311G" + str(lambda_list[i]).replace(".", "_"), r_array)
