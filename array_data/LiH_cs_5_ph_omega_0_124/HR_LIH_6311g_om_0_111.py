...
np.save("cs_5_ph_om_0_124_fci_cavity_free_array_LIH_6311g", cavity_free_E_array)
...
for i in range(0,len(lambda_values)):
 ...
 #For every line, change MOLECULE to the molecule of interest (and basis set if not at 6-311G) 
 np.save("cs_5_ph_om_0_124_fci_cavity_arrays_LIH_6311g"+ str(lambda_values[i]).replace(".","_"), cavity_E_array)
 np.save("cs_5_ph_om_0_124_fci_dipoles_LIH_6311g"+str(lambda_values[i]).replace(".","_"), Dipoles)
np.save("cs_5_ph_om_0_124_fci_r_array_LIH_6311G", r_data)
